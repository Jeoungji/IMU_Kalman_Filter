...
bool MYMPU9250::WriteReg( uint8_t WriteAddr, uint8_t WriteData )
{
    select();
...
uint8_t MYMPU9250::ReadReg( uint8_t ReadAddr)
{
...
void MYMPU9250::ReadRegs( uint8_t ReadAddr, uint8_t *ReadBuf, unsigned int Bytes )
{
    unsigned int  i = 0;
...
bool MYMPU9250::init(bool calib_gyro, bool calib_acc){
...
int MYMPU9250::auto_init(uint8_t AccelScale, uint8_t GyroScale) {
    spi->begin();
    delayMicroseconds(1000);
    ...
    a_bias[0] = 0;//-2.9324521031697503;
    a_bias[1] = 0;//-2.8258385682423893;
    a_bias[2] = 0;//-4.9365387217203693;

    a_scale[0] = 1;
    a_scale[1] = 1;//0.998457501791564;
    a_scale[2] = 1;//0.987252408576916;

    g_bias[0] = 0;//7.4575668138901366;
    g_bias[1] = 0;//-7.3655416125853782;
    g_bias[2] = 0;//4.891091213953488;
    
    g_scale[0] = 1;//0.353808809007075;
    g_scale[2] = 1;
    g_scale[3] = 1;

    memset(&a_angle_bias.Roll, 0, sizeof(a_angle_bias));


    a_angle_bias.Roll = 0;//2.8227101144313813;
    a_angle_scale[0][0] = 1;//90/84.26783945617676; // Roll +방향 스케일 
    a_angle_scale[0][1] = 1;//90/89.92471615142823; // Roll -방향 스케일 
    a_angle_scale[1][0] = 1; // Pitch +방향 스케일 
    a_angle_scale[1][1] = 1; // Pitch -방향 스케일 
    a_angle_scale[2][0] = 1; // Yaw +방향 스케일 
    a_angle_scale[2][1] = 1; // Yaw -방향 스케일 
    ...
    //memset(a_scale, 1, 3*sizeof(float));
...
uint16_t MYMPU9250::set_acc_scale(uint8_t scale){
    uint8_t temp_scale;
...
uint16_t MYMPU9250::set_gyro_scale(uint8_t scale){
    uint8_t temp_scale;
    uint16_t return_scale;
...
void MYMPU9250::read_gyro()
{
    ...
    for(i = 0; i < 3; i++) {
        bit_data = ((int16_t)response[i*2]<<8) | response[i*2+1];
        gyro_lowdata[i] = (float)bit_data/gyro_divider;
        gyro_data[i] = gyro_lowdata[i] - g_bias[i];
    }
...
void MYMPU9250::read_all(){
    read_acc();
    read_gyro();
    read_temp();
}

void MYMPU9250::calibrate(float *dest1, float *dest2){  
...
bool MYMPU9250::accel_calibrate(int sample) {
#if MPU9250DEBUG==1 && DEBUGINFO==1
...
bool MYMPU9250::gyro_calibrate(int sample) {
...
bool MYMPU9250::accelangle_calibrate(int sample) {
...
void MYMPU9250::CarculateAngle() {
    float roll = atan(accel_data[1]/sqrt(pow(accel_data[0],2)
     + pow(accel_data[2],2)))*1/ (PI/180) - a_angle_bias.Roll;//- 5.07;
    if (roll >= 0)  roll = roll * a_angle_scale[0][0];
    else            roll = roll * a_angle_scale[0][1];
    accelangle.Roll = roll;

    float pitch = -atan(accel_data[0]/sqrt(pow(accel_data[1],2)
     + pow(accel_data[2],2)))*1/ (PI/180) - a_angle_bias.Pitch;// + 6.54;
    
    if (pitch >= 0) pitch = pitch * a_angle_scale[0][0];
    else            pitch = pitch * a_angle_scale[0][1];
    accelangle.Pitch = pitch;
}

void MYMPU9250::CarculateAngleGyro(float deltatime){
    gyroangle.Roll += g_scale[0] * gyro_data[0] * deltatime;
    gyroangle.Pitch += g_scale[1] * gyro_data[1] * deltatime;
    gyroangle.Yaw += g_scale[2] * gyro_data[2] * deltatime; 
}


void MYMPU9250::kalman_1d(float KalmanState, float KalmanUncertainty,
    ...
    */
{
    float S = 46;//4;
    float A = 1*1; // 시스템 행렬
    float Q = pow(S*deltatime,2); // 시스템 잡음 공분산 (측정된 주기가 길어지면 오차가 증가)
    float H = 1*1; // 출력 행렬, 측정값과 상태변수의 관계
    float R = 20*20;//3*3; // 측정 잡음 공분산
...
ANGLE MYMPU9250::CalKalmanAngle(uint32_t time) {
    ...
    // Roll
    kalman_1d(KalmanAngle.Roll, KalmanUncertaintyAngle.Roll, 
              gyro_data[0] * g_scale[0], accelangle.Roll, deltatime);
    KalmanAngle.Roll= Kalman1DOutput[0];
    KalmanUncertaintyAngle.Roll = Kalman1DOutput[1];
    // Pitch
    kalman_1d(KalmanAngle.Pitch, KalmanUncertaintyAngle.Pitch, 
              gyro_data[1] * g_scale[1], accelangle.Pitch, deltatime);
...
void MYMPU9250::MeaurementAccelBias(int sample) {
    double x1bias[3] = {0,};
    double x2bias[3] = {0,};
    double y1bias[3] = {0,};
    double y2bias[3] = {0,};
